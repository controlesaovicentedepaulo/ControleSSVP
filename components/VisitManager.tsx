...
import { Footprints, Plus, Search, Calendar, User, MessageCircle, Edit, X, ChevronDown, Check, Info, ShieldCheck, UserCog, Trash2, AlertTriangle, FileDown, ArrowUpDown, ArrowUp, ArrowDown } from 'lucide-react';
...
const VisitManager: React.FC<VisitManagerProps> = ({ visits, families, members, onRefresh }) => {
  ...
  const [sortColumn, setSortColumn] = useState<'data' | 'familia' | 'motivo'>('data');
  ...
  const handleSort = (column: 'data' | 'familia' | 'motivo') => {
  ...
  // Ordenar visitas baseado na coluna e direção selecionadas
  const sortedVisits: Visit[] = useMemo(() => {
    return [...filteredVisits].sort((a, b) => {
      ...
      switch (sortColumn) {
        case 'data':
          ...
          break;
        case 'familia':
          comparison = getFamilyName(a.familyId).localeCompare(getFamilyName(b.familyId), 'pt-BR');
          break;
        case 'motivo':
          comparison = (a.motivo || '').localeCompare(b.motivo || '', 'pt-BR');
          break;
      }
            ...
            <tr className="bg-slate-50 border-b border-slate-100">
              <th className="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase">
                <button
                  ...
                  className={`flex items-center gap-1.5 hover:text-emerald-600 transition-colors group/sort ${sortColumn === 'data' ? 'text-emerald-600' : ''}`}
                  title="Ordenar por data"
                >
                  <span>Data</span>
                  {sortColumn === 'data' ? (
                    sortDirection === 'asc' ? <ArrowUp size={12} className="text-emerald-600" /> : <ArrowDown size={12} className="text-emerald-600" />
                  ) : (
              ...
              <th className="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase">
                <button
                  ...
                  className={`flex items-center gap-1.5 hover:text-emerald-600 transition-colors group/sort ${sortColumn === 'familia' ? 'text-emerald-600' : ''}`}
              ...
              <th className="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase">
                <button
                  ...
                  className={`flex items-center gap-1.5 hover:text-emerald-600 transition-colors group/sort ${sortColumn === 'motivo' ? 'text-emerald-600' : ''}`}
